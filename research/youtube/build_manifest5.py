#!/usr/bin/env python3
"""config5 소스들의 영상목록 + 실제 업로드 날짜 매니페스트 생성.

폴더별(BASE/<folder>) manifest.jsonl 저장 (한 폴더에 여러 group 가능).
날짜는 전 고유 영상ID 단일 프로세스 순차(배치) 조회.
"""
import json
import subprocess
...
def main():
    ...
    for s in SOURCES:
        ...
        recs = folder_recs.setdefault(s["folder"], [])
        for i, v, t in rows:
            recs.append({"folder": s["folder"], "group": s["group"],
                         "index": i, "id": v, "title": t, "upload_date": None})
            all_ids.add(v)

    print(f"\n고유 영상 {len(all_ids)}개 날짜 조회", flush=True)
    dates = fetch_dates(sorted(all_ids))

    for folder, recs in folder_recs.items():
        for r in recs:
        ...
        ds = [r["upload_date"] for r in recs if r["upload_date"]]
        print(f"[{folder}] 항목 {len(recs)} | 2018={c.get('2018',0)} "
              f"2024={c.get('2024',0)} | 미상={c.get('????',0)} | "
              f"범위 {min(ds, default='-')}~{max(ds, default='-')}", flush=True)
    print("완료", flush=True)
